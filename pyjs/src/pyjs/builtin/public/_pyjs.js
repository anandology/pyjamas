...
function $pyjs__class_instance(class_name, module_name) {
    ...
    var cls_fn = function(){
        if (cls_fn.__number__ === null) {
            ...
            var instance = cls_fn.__new__.apply(null, args);
        } else {
            var instance = cls_fn.__new__.apply(null, [cls_fn, arguments]);
        }
        if (cls_fn.__new__.__$pyjs_autogenerated__ && typeof instance.__init__ == 'function') {
...
function $pyjs_type(clsname, bases, methods)
{
    ...
    for (var i in methods) {
        if (typeof methods[i] == 'function' && methods[i].__class__ === undefined) {
            if (methods[i].__bind_type__ !== undefined) {
